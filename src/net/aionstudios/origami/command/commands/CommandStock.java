...
public class CommandStock extends OrigamiCommand {
	...
	public void publicCall(String[] args, String message, MessageReceivedEvent e) {
		...
		Stock s;
		try {
			s = StockFetcher.getStock(args[1]);
			String change;
			...
			if(s.getName()==null||s.getName()=="") {
				e.getChannel().sendMessage(ErrorEmbedGenerator.genError("Stock ("+args[1]+"): NO SUCH STOCK")).queue();
			} else {
				e.getChannel().sendMessage("Stock ("+args[1]+"): "+s.getName()+"  "+s.getPrice()+" "+s.getCurrency()+"  "+change).queue();
			}
		} catch (IOException e1) {
			e.getChannel().sendMessage(ErrorEmbedGenerator.genError("Failed while connecting to service - 'https://query.yahooapis.com'")).queue();
			e1.printStackTrace();
		}
	}
